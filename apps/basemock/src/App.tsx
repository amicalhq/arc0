...
import { InputModal, SelectModal, ConfirmModal, WalkModal } from './components/InputModal.js';
import { logger, type LogEntry } from './logger.js';
import type { CategoryKey, StatusType, MockSession, ClaudeJSONLMessage, ToolUseBlock, JSONLPayload } from './types.js';
import {
  startServer,
  stopServer,
  isServerRunning,
...
type ModalType =
  | { type: 'input'; title: string; placeholder: string; initialValue?: string; onSubmit: (value: string) => void }
  | { type: 'select'; title: string; options: Array<{ value: string; label: string; hint?: string }>; onSelect: (value: string) => void }
  | { type: 'confirm'; title: string; message: string; onConfirm: (confirmed: boolean) => void }
  | { type: 'walk'; title: string; message: string; step: number; total: number; sessionId: string }
  | null;
...
const WALK_STEPS: WalkStep[] = [
  { name: 'User text', create: (sid) => createUserTextMessage(sid, 'How do I add a product to the cart?') },
  { name: 'Assistant text', create: (sid) => createAssistantTextMessage(sid, 'I can help you add products to the cart.') },
  { name: 'Assistant + thinking', create: (sid) => createAssistantWithThinking(sid, 'The user wants to add products to their cart. I should explain the cart API...', 'Here is how to add products to the cart.') },
  { name: 'Thinking only', create: (sid) => createThinkingOnlyMessage(sid, 'Analyzing the checkout flow... considering options...') },
  { name: 'Read tool (auto)', create: (sid) => createToolUseRead(sid, '/home/user/acme-app/src/index.ts') },
  { name: 'Grep tool (auto)', create: (sid) => createToolUseGrep(sid, 'TODO|FIXME') },
  { name: 'Write tool (approval)', create: (sid) => createToolUseWrite(sid, '/home/user/acme-app/src/new-file.ts', 'export const hello = "world";') },
  { name: 'Bash tool (approval)', create: (sid) => createToolUseBash(sid, 'npm run build') },
  { name: 'Tool result (success)', create: (sid) => createToolResult(sid, 'toolu_walk_test', 'Command executed successfully') },
  { name: 'Tool result (error)', create: (sid) => createToolResult(sid, 'toolu_walk_err', 'Error: File not found', true) },
  { name: 'AskUser (simple)', create: (sid) => createAskUserQuestion(sid, 'Which database?', ['PostgreSQL', 'SQLite', 'MongoDB']) },
  { name: 'AskUser (multi-select)', create: (sid) => createAskUserQuestionMultiSelect(sid, 'Which features?', 'Features', [
    { label: 'Auth', description: 'User authentication' },
    { label: 'API', description: 'REST API endpoints' },
    { label: 'Tests', description: 'Unit tests' },
  ]) },
  { name: 'AskUser (multi-question)', create: (sid) => createAskUserQuestionFeatureConfig(sid) },
  { name: 'ExitPlanMode', create: (sid) => createExitPlanMode(sid, 'Step 1: Set up\nStep 2: Implement\nStep 3: Test') },
  { name: 'TodoWrite', create: (sid) => createTodoWrite(sid, [
    ...
    { content: 'Third task', status: 'pending', activeForm: 'Third task pending' },
  ]) },
  { name: 'Custom title (rename)', create: (sid) => createCustomTitleMessage(sid, 'Demo Session') },
  { name: 'Tool permission (Bash)', create: (sid) => createToolUsePermission(sid, 'npm test', 'Run tests') },
];
...
export function App(): React.ReactElement {
  ...
  const [currentSession, setCurrentSessionState] = useState<MockSession | null>(getCurrentSession());
    ...
    switch (action) {
      ...
      case 'user-text': {
        if (!serverRunning || !currentSession) {
          setStatus(serverRunning ? 'No active session' : 'Server not running', 'error');
          return;
        }
        setModal({
          type: 'input',
          ...
          onSubmit: (thinking) => {
            ...
            setModal({
              ...
              onSubmit: async (text) => {
                ...
                const msg = createAssistantWithThinking(currentSession.id, thinking, text);
                await sendMessagesBatch(currentSession.id, [msg]);
      ...
      case 'thinking-only': {
        ...
        setModal({
          ...
          onSubmit: async (thinking) => {
            ...
            const msg = createThinkingOnlyMessage(currentSession.id, thinking);
            await sendMessagesBatch(currentSession.id, [msg]);
      ...
      case 'sample-conversation': {
        ...
        logger.success('Sample conversation sent', `${messages.length} messages`);
        ...
        setModal({
          ...
          onSubmit: (filePath) => {
            ...
            setModal({
              ...
              initialValue: 'export function hello() {\n  return "Hello!";\n}',
              onSubmit: async (content) => {
                if (!content) {
                  closeModal();
                  return;
                }
                const msg = createToolUseWrite(currentSession.id, filePath, content);
                await sendMessagesBatch(currentSession.id, [msg]);
              ...
              onSubmit: (result) => {
                ...
                setModal({
                  ...
                  onConfirm: async (isError) => {
                    const msg = createToolResult(currentSession.id, toolUseId, result, isError);
                    await sendMessagesBatch(currentSession.id, [msg]);
          ...
          onSubmit: (question) => {
            ...
            setModal({
              ...
              onSubmit: async (optionsStr) => {
                ...
                const msg = createAskUserQuestion(currentSession.id, question, options);
                await sendMessagesBatch(currentSession.id, [msg]);
      ...
      case 'exit-plan': {
        ...
        setModal({
          ...
          onSubmit: async (planContent) => {
            const msg = createExitPlanMode(currentSession.id, planContent || undefined);
            await sendMessagesBatch(currentSession.id, [msg]);
            logger.success('ExitPlanMode sent', planContent ? 'with plan' : 'no plan');
        ...
        setModal({
          ...
          onSubmit: async (countStr) => {
            ...
            if (!isNaN(count) && count > 0) {
              ...
              logger.success('TodoWrite sent', `${count} auto-generated tasks`);
            } else {
              // Use default sample tasks
              msg = createTodoWrite(currentSession.id, [
                { content: 'Set up Acme app structure', status: 'completed', activeForm: 'Setting up project' },
                { content: 'Implement product catalog', status: 'in_progress', activeForm: 'Implementing catalog' },
              ...
              onSubmit: (command) => {
                ...
                setModal({
                  ...
                  onSubmit: async (description) => {
                    ...
                    await sendMessagesBatch(currentSession.id, [msg]);
                    logger.success('Tool permission sent', shouldError ? '(ack will error)' : '(ack will succeed)');
      ...
      case 'tool-auto-approved': {
        ...
        const toolUseMsg = createToolUseBash(currentSession.id, 'npm install moment');
        ...
        await sendMessagesBatch(currentSession.id, [toolUseMsg, toolResultMsg]);
      ...
      case 'tool-approve-delayed': {
        ...
        const toolUseMsg = createToolUseBash(currentSession.id, 'npm install lodash');
        const toolUseId = extractToolUseId(toolUseMsg);
        if (!toolUseId) {
          setStatus('Failed to create tool_use', 'error');
          return;
      ...
      case 'tool-reject-delayed': {
        ...
        const toolUseMsg = createToolUseBash(currentSession.id, 'npm install left-pad');
        const toolUseId = extractToolUseId(toolUseMsg);
        if (!toolUseId) {
          setStatus('Failed to create tool_use', 'error');
          return;
  ...
  useInput((input, key) => {
    ...
    if (key.pageUp) {
      setLogScrollOffset((prev) => Math.min(prev + LOG_MAX_VISIBLE, logMaxOffset));
      return;
    }
    if (key.pageDown) {
  ...
  return (
    <Box flexDirection="row" width="100%" height="100%">
      ...
      <Box flexDirection="column" width="60%">
        ...
        />
        <LogViewer logs={logs} maxVisible={LOG_MAX_VISIBLE} scrollOffset={logScrollOffset} />
      </Box>
          ...
          <WalkModal
            ...
            onNext={async () => {
              ...
              if (nextStep < WALK_STEPS.length) {
                ...
                logger.success('Walk completed', `${WALK_STEPS.length} message types sent`);
                setStatus('Walk completed!', 'success');
                closeModal();
              }
            }}
            onCancel={() => {
              logger.info('Walk cancelled', `Stopped at step ${modal.step + 1}`);
